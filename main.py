...
def test(relevant_feature_count=10, total_feature_count=200, iterations=1000):
    ...
    def run_hics(steered: Optional[bool] = True, steered_exploration_weight=.3) -> List[float]:
...
def test_dataset(relevant_feature_count: int, total_feature_count: int, dataset_size=10000,
                 mutual_information=.7, slight_mutual_information=.6, noise_mutual_information=0):
    def exponential_distribution() -> List[float]:
        return [((1 - x / 10) if x < .3 else exp(-10 * x)) for x in numpy.linspace(0, 1, num=total_feature_count)]
    ...
    for i in range(total_feature_count):
        features[f"{'mutual' if i < relevant_feature_count else 'noise'}-{i}"] = feature(distribution[i])
